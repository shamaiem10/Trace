"""Train reproducible raw and NHANES population-reweighted research models."""
...
DISCLAIMER = "This is a research model score, not a diagnosis. It is not medical advice."
...
def representation_audit_payload(audit) -> dict:
    """Serialize the population-comparison table for the dashboard and model card."""
    strata = []
    for row in audit.findings.to_dict(orient="records"):
        strata.append({"bmi_category": str(row["bmi_cat"]),
                       "menstrual_irregularity": int(row["Menstrual_Irregularity"]),
                       "source_rate": float(row["source_rate"]),
                       "reference_rate": float(row["reference_rate"]),
                       "weight_ratio": None if pd.isna(row["ratio"]) else float(row["ratio"])})
    return {"reference_population": "NHANES 2021-2023 survey-weighted reference extract",
            "matching_variables": ["BMI category", "menstrual irregularity"], "strata": strata}
...
def train(data_dir: Path, artifact_dir: Path) -> dict:
    ...
    train_idx, test_idx = train_test_split(indices, test_size=0.2, stratify=source.Diagnosis, random_state=42)
    fit_idx, calibration_idx = train_test_split(train_idx, test_size=0.2, stratify=source.loc[train_idx, "Diagnosis"], random_state=42)
    # Fit weights are learned without inspecting calibration or held-out rows.
    weighted_fit, audit = audit_and_reweight(source.loc[fit_idx], reference, "WTINT2YR", [("bmi_cat", "bmi_cat"), ("Menstrual_Irregularity", "irregular")])
    # The public representation table describes the whole supplied source only;
    # it is never used for model fitting.
    _, representation_audit = audit_and_reweight(source, reference, "WTINT2YR", [("bmi_cat", "bmi_cat"), ("Menstrual_Irregularity", "irregular")])
    np.savez(artifact_dir / "split_indices.npz", fit=fit_idx, calibration=calibration_idx, test=test_idx)
    x_fit, x_test = source.loc[fit_idx, FEATURES], source.loc[test_idx, FEATURES]
    y_fit, y_test = source.loc[fit_idx, "Diagnosis"], source.loc[test_idx, "Diagnosis"]
    params = dict(random_state=42, max_depth=4, max_iter=150)
    raw_model = HistGradientBoostingClassifier(**params).fit(x_fit, y_fit)
    corrected_model = HistGradientBoostingClassifier(**params).fit(x_fit, y_fit, sample_weight=weighted_fit["cal_weight"])
    raw = raw_model.predict_proba(x_test)[:, 1]
    corrected = corrected_model.predict_proba(x_test)[:, 1]
    residuals = np.abs(source.loc[calibration_idx, "Diagnosis"] - corrected_model.predict_proba(source.loc[calibration_idx, FEATURES])[:, 1])
    ...
    shap_background = source.loc[fit_idx, FEATURES].sample(n=min(SHAP_BACKGROUND_ROWS, len(fit_idx)), random_state=42)
    ...
    summary = {"schema_version": 3, "project_name": "Trace", "features": FEATURES, "disclaimer": DISCLAIMER,
               ...
               "model_variant": "NHANES population-reweighted (not probability calibrated)",
               "explanation_reference": explanation_reference(source.loc[fit_idx]),
               ...
               "representation_audit": representation_audit_payload(representation_audit),
